 ...
 * itself).
 *
 * It is required that an applicationContext is provided, and also required that
 * either a location or an inputStream (but not both) are provided. It is
 * optional that a resources and/or charset is provided.
 *
 * Simple usage:
 *
 * <pre>
 * Parent root = SpringFXMLLoader.create().applicationContext(myAppContext)
 * 		.location(getClass().getResource("myApp.fxml")).load();
 * </pre>
 *
 * If access to the controller is needed, a reference must be kept to the
 * SpringFXMLLoader:
 *
 * <pre>
 * SpringFXMLLoader<Parent, MyControllerClass> loader = SpringFXMLLoader.create().applicationContext(myAppContext)
 * 		.location(getClass().getResource("myApp.fxml")).build();
 * Parent root = loader.load();
 * MyController controller = loader.getController();
 * </pre>
 *
 * @param <S>
 *            The type of the root element of the FXML.
 * @param <T>
...
public class SpringFXMLLoader<S, T> {
	...
	private SpringFXMLLoader(ApplicationContext applicationContext, URL location, InputStream inputStream,
			ResourceBundle resources, Charset charset) {
	...
	public S load() throws IOException {
		...
		loader.setBuilderFactory(new BuilderFactory() {
			...
			public javafx.util.Builder<?> getBuilder(Class<?> type) {
				final String[] beanNames = applicationContext.getBeanNamesForType(type);
